'use strict';
/*
	to add a new front end app create new entry (around line 13) and update
	entry property for development and production settings
*/
...
var SANDBOX_ENTRY = path.join(config.paths.source, 'frontend-apps/sandbox/sandbox-entry'); // dev only entry

var COUNTER_ENTRY = path.join(config.paths.source, 'frontend-apps/counter/counter-entry');
var ABOUT_ENTRY = path.join(config.paths.source, 'frontend-apps/about/about-entry');

var WEBPACK_HOT_ENTRY = 'webpack-hot-middleware/client?path=' + config.webpack.devServerUrl + '/__webpack_hmr&timeout=20000';
var JS_JSX = /\.(js|jsx)$/;
...
if (process.env.NODE_ENV === 'development') {
	Object.assign(webpackConfig, {
		entry: {
			sandbox: [SANDBOX_ENTRY, WEBPACK_HOT_ENTRY], // dev only
			counter: [COUNTER_ENTRY, WEBPACK_HOT_ENTRY],
			about: [ABOUT_ENTRY, WEBPACK_HOT_ENTRY],
		},
		devtool: '#inline-source-map', // Generate source maps (more or less efficiently)
		module: {
			preLoaders: [
				{
					test: JS_JSX,
